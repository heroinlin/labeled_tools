...
# 目标框标注程序
class CLabeled:
    def __init__(self, file_folder, data_list_path=None):
        # 存放需要标注图像的文件夹
        ...
        # 记录历史标注位置的文本文件地址
        self.checkpoint_path = os.path.join(file_folder, "checkpoint")
    ...
    # 统计所有图片个数
    def _compute_total_file_number(self):
        self.total_file_number = len(self.file_list)

    # 判断是否需要新建文件夹
    def _may_make_dir(self):
        if not os.path.exists(self.file_folder):
            print(self.file_folder, " does not exists! please check it !")
            exit(-1)
        path = os.path.join(self.file_folder, "labels")
    ...
    def _get_file_list(self):
        ...
        if len(self.data_list_path):
            ...
            self.file_list = [file_path.strip().split(" ")[0].replace("/home", "\\\\\\\\192.168.1.177/aigroup") 
                                for file_path in self.file_list if len(file_path.strip().split(" "))]
            print(self.file_list)
        else:
            self.file_list = sorted(self._get_file_from_root_dir(self.file_folder, ext=['jpg', 'png']))
    ...
    def _update_win_image(self, image):
        ...
        cv2.putText(self.win_image,
                    f"{self.current_label_index}/{self.total_file_number}",
    ...
    # 将标注框信息保存到文本
    def write_label_file(self, label_file_path, total_frame_num=0):
    ...
    def labeled(self):
        self._check()
        print(os.path.abspath(self.file_folder))
        self._get_file_list()
       
        self._compute_total_file_number()
        print("需要标注的文件总数为： ", self.total_file_number)
        if os.path.exists(self.checkpoint_path):
            self.read_checkpoint(self.checkpoint_path)
        
        while self.current_label_index < self.total_file_number:
            print("已标注的文件数量为： ", self.current_label_index)
            ...
            file_path = self.file_list[self.current_label_index]
            # print(file_path)
            
            path_file_name = os.path.splitext(file_path)[0]
            ...
            self.image = cv2.imdecode(np.fromfile(file_path, np.uint8), 1)
            image_height , image_width = self.image.shape[0:2]
            h_w_rate = image_height / image_width
            ...
            key = cv2.waitKeyEx(int(self.decay_time))  # 开启方向键功能
            ...
            if key == ord('l') or key == ord('L'):  # 删除当前文件及标注文件
                os.remove(self.file_list[self.current_label_index])
                os.remove(self.label_path)
                del self.file_list[self.current_label_index]
                self._compute_total_file_number()
                # self._backward()
                continue
            if key == 27:  # 退出
